...
p_FragLen <- ggplot(fragment_size_vector_df, aes(x=Size))+
  ...
  theme(legend.position="bottom",
        ...
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank())
ggsave(plot=p_FragLen, sprintf("%s/FragmentSize_NBT_20230316.pdf", fig.directory), width=10, height=5)
# ggsave(plot=p_FragLen, sprintf("%s/FragmentSize_NBT_20230331.pdf", fig.directory), width=60, height=30, units="mm")


#############  Plot: Error rate ############# 
# Figure 2 Panel b Middle
...
p_errorrate <- ggplot() +
  ...
  theme(legend.position="none",
        ...
        panel.grid.minor = element_blank())
p_errorrate <- ggplot() +
  geom_line(data = error_rate_df %>% filter(group=="read 1"), aes(x = position, y = error_rate, color = method, group = group), size=1.5, alpha=0.5) +
  geom_line(data = error_rate_df %>% filter(group=="read 2"), aes(x = position, y = error_rate, color = method, group = group), size=0.7, alpha=0.5) +
  scale_x_continuous(limits = c(0, 49)) +
  xlab(sprintf("position in read")) +
  ylab("error rate") +
  ...
  theme(legend.position="none",
        ...
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank())
ggsave(plot=p_errorrate, sprintf("%s/Errorate_plot_NBT_20230326.pdf", fig.directory), width=10, height = 4)
