...
## Set directory
out.directory <- "/home/gayan/Projects/scATAC_Simulator/results/20221130_e18_mouse_brain_fresh_5k_atac_possorted_bam_chr1_NONINPUT"
...
filename <- "e18_mouse_brain_fresh_5k_atac_possorted_bam_chr1_NONINPUT"
...
fdrPeakSelector <- function(synthetic_peak_sorted, real_peak, overlap_size, FDR_control){
  # synthetic_peak_sorted: synthetic peak list with power decreasing order
  # Count total regions number
  real_peak_GR <- makeGRangesFromDataFrame(as.data.frame(real_peak))
  synthetic_peak_GR <- makeGRangesFromDataFrame(as.data.frame(synthetic_peak_sorted))
  ...
  # Find threshold value corresponding to FDR_control value
  FDR_tocontrol_distance <- abs(FDR_vec - FDR_control)
  FDRcontrol_ind <- which(FDR_tocontrol_distance == min(FDR_tocontrol_distance))[1]
  threshold_quantile_FDRcontrol <- threshold_quantile_list[FDRcontrol_ind]
  print(FDR_vec[FDRcontrol_ind])
  
  TN_FDRcontrol <- round(threshold_quantile_FDRcontrol * nrow(synthetic_peak_sorted))
  synthetic_peak_trun_FDRcontrol <- synthetic_peak_sorted[1:(nrow(synthetic_peak_sorted) - TN_FDRcontrol),]
  return(list(TPR_vec=TPR_vec, FDR_vec=FDR_vec, synthetic_peak_trun_FDRcontrol=synthetic_peak_trun_FDRcontrol))
}

############# Plot 1 Venn Plot ############# 
# Figure S7 Panel c
real_peakfile <- sprintf("%s/e18_mouse_brain_fresh_5k_atac_possorted_bam_chr1.MACS3_peaks.bed", out.directory)
# synthetic_peakfile <- sprintf("%s/e18_mouse_brain_fresh_5k_atac_possorted_bam_chr1.syntheticBAM.CBincluded.synthetic.chr1.sorted.bam.MACS3.lessStringent_peaks.narrowPeak", out.directory)
# synthetic_peakfile <- sprintf("%s/e18_mouse_brain_fresh_5k_atac_possorted_bam_chr1.syntheticBAM.CBincluded.synthetic.chr1.sorted.bam.MACS3_peaks.narrowPeak", out.directory)
synthetic_peakfile <- sprintf("%s/e18_mouse_brain_fresh_5k_atac_possorted_bam_chr1.syntheticBAM.CBincluded.synthetic.chr1.sorted.bam.MACS3.stringent01_peaks.narrowPeak", out.directory)
...
ggsave(venn_plot, file=sprintf("%s/VennPlot_PeakCalling_plot_NBT.20230316.pdf", fig.directory), height = 4, width = 4)


############# Plot 2 FPR & FDR Plot ############# 
# Figure 2 Panel b Bottom
real_peakfile <- sprintf("%s/e18_mouse_brain_fresh_5k_atac_possorted_bam_chr1.MACS3_peaks.bed", out.directory)
synthetic_peakfile <- sprintf("%s/e18_mouse_brain_fresh_5k_atac_possorted_bam_chr1.syntheticBAM.CBincluded.synthetic.chr1.sorted.bam.MACS3.lessStringent_peaks.narrowPeak", out.directory)
# synthetic_peakfile <- sprintf("%s/e18_mouse_brain_fresh_5k_atac_possorted_bam_chr1.syntheticBAM.CBincluded.synthetic.chr1.sorted.bam.MACS3_peaks.narrowPeak", out.directory)
...
curve_plot <- annotate_figure(curve_plot, top = text_grob("Consistency of MACS3's peak calling results\n on real and synthetic data", 
                                                          size = 25))
ggsave(file=sprintf("%s/Fig.VeryRead.Peak.Combined.NBT.20230316.pdf",fig.directory), curve_plot, height =5 , width = 9.6)
