...
out_directory <- "/home/gayan/Projects/scATAC_Simulator/results/20221011_e18_mouse_brain_fresh_5k_gex_possorted_bam_chr1_NONINPUT/verification_UMI_count"
...
save(clustering_synthetic_cluster, file=sprintf("%s/BenchmarkUMI.synthetic_cell_barcode.formatted.20230320.Rdata", out_directory))
...
save(processed_scDesign2, processed_cellranger, processed_Alevin, processed_umitools, file=sprintf("%s/Filtered.withAlevin.MatchedProcessedUMIMat_20221013.Rdata", out_directory))
save(intersect_gene_ID, intersect_cell_ID, file=sprintf("%s/Filtered.withAlevin.MatchedProcessedUMIMat.GeneIDandCellID_20221015.Rdata", out_directory))

## TODO
## 1. Add two metrics: # identified chr1 gene and running time
load(sprintf("%s/Filtered.withAlevin.MatchedProcessedUMIMat_20221013.Rdata", out_directory))
...
}), use.names=FALSE) # 908 nonzeros
# cor_cellranger[is.na(cor_cellranger)] <- 0
...
# cor_umitools[is.na(cor_umitools)] <- 0
...
save(cor_cellranger, cor_Alevin, cor_umitools, Kcor_cellranger, Kcor_Alevin, Kcor_umitools, file=sprintf("%s/VerifyPipeline.Quantity.CorrelationPerGene_20221013.Rdata", out_directory))
...
save(cor_cellranger.percell, cor_Alevin.percell, cor_umitools.percell, Kcor_cellranger.percell, Kcor_Alevin.percell, Kcor_umitools.percell, file=sprintf("%s/VerifyPipeline.Quantity.CorrelationPerCell_20221013.Rdata", out_directory))
...
save(cor_cellranger.global, cor_Alevin.global, cor_umitools.global, file=sprintf("%s/VerifyPipeline.Quantity.Global.Correlation_20221013.Rdata", out_directory))

########################### Fig A. Correlation  ###########################
cor_df <- data.frame(value=c(cor_cellranger, cor_Alevin, cor_umitools, cor_cellranger.percell, cor_Alevin.percell,cor_umitools.percell,
                             Kcor_cellranger, Kcor_Alevin, Kcor_umitools, Kcor_cellranger.percell, Kcor_Alevin.percell,Kcor_umitools.percell),
                     measure=c(rep("gene-wise", 3*length(cor_cellranger)), rep("cell-wise", 3*length(cor_cellranger.percell)),
                               rep("gene-wise", 3*length(cor_cellranger)), rep("cell-wise", 3*length(cor_cellranger.percell))),
                     method=c(rep("cellranger", length(cor_cellranger)), rep("Alevin", length(cor_cellranger)), rep("UMI-tools", length(cor_cellranger)), rep("cellranger", length(cor_cellranger.percell)), rep("Alevin", length(cor_cellranger.percell)), rep("UMI-tools", length(cor_cellranger.percell)),
                              rep("cellranger", length(cor_cellranger)), rep("Alevin", length(cor_cellranger)), rep("UMI-tools", length(cor_cellranger)), rep("cellranger", length(cor_cellranger.percell)), rep("Alevin", length(cor_cellranger.percell)), rep("UMI-tools", length(cor_cellranger.percell))),
                     cor=c(rep("Pearson cor.", 3 * (length(cor_Alevin) + length(cor_Alevin.percell))),
                           rep("Kendall's tau", 3 * (length(cor_Alevin) + length(cor_Alevin.percell)))))
cor_df$method <- factor(cor_df$method, levels=c("cellranger", "Alevin", "UMI-tools"))
...
save(cor_df, file=sprintf("%s/BenchmarkUMI_Fig1_Correlation_20221013.Rdata", out_directory))

load(file=sprintf("%s/BenchmarkUMI_Fig1_Correlation_20221013.Rdata", out_directory))
...
cor_plot <- cor_df %>%
  ...
  theme_bw() +
  scale_fill_manual(values=group.colors[-1]) +
  theme(
...
pdf(sprintf("%s/BenchmarkUMI_Fig1_Correlation_20221013.pdf",out_directory))
print(cor_plot)
dev.off()
...
stats_dat <- rbind(stats_scDesign2, stats_umitools, stats_alevin, stats_cellranger)
stats_dat$group <- factor(stats_dat$group, levels = c('Ground truth','cellranger', 'Alevin', 'UMI-tools'))
...
save(stats_dat, file=sprintf("%s/BenchmarkUMI_Fig2_SummaryStats_20221013.Rdata", out_directory))

# create violin-plots to compare the marginal stat values -------------------------------
load(sprintf("%s/BenchmarkUMI_Fig2_SummaryStats_20221013.Rdata", out_directory))
group.colors <- c("#D55E00", "#E69F00", "#56B4E9", "#009E73") # groudtruth, cellranger, Alevin, UMI-tools
stats_plot <- ggplot(stats_dat, aes(x = group, y = value, fill=group)) +
  ...
  xlab("") + ylab("")
ggsave(plot=stats_plot, file=sprintf("%s/BenchmarkUMI_Fig2_SummaryStats_20221013.pdf",out_directory), height = 4.8, width = 9.6, dpi = 600)
...
Alevin.filtered.id <- filter_nonzero_id(processed_Alevin, filter_thr)
cellranger.filtered.id <- filter_nonzero_id(processed_cellranger, filter_thr)
# Keep consensus peaks
gene.filterd.id <- Reduce(intersect, list(scDesign2.filtered.id, umitools.filtered.id, Alevin.filtered.id, cellranger.filtered.id)) 
...
Alevin.processed.matrix <- logtrans(processed_Alevin[gene.filterd.id,])
cellranger.processed.matrix <- logtrans(processed_cellranger[gene.filterd.id,])
save(gene.filterd.id, scDesign2.processed.matrix, umitools.processed.matrix, Alevin.processed.matrix, cellranger.processed.matrix, file=sprintf("%s/BenchmarkUMI.withAlevin.Fig3_UMAP.ManualDR.RealPCASpace.ProcessedData.20230320.Rdata", out_directory))
...
Alevin.pcatfidf <- t(t(Alevin.processed.matrix) %*% scDesign2_pca$v) # K by n
cellranger.pcatfidf <- t(t(cellranger.processed.matrix) %*% scDesign2_pca$v) # K by n
save(scDesign2_pca, scDesign2.pcatfidf, umitools.pcatfidf, Alevin.pcatfidf, cellranger.pcatfidf, file=sprintf("%s/BenchmarkUMI.withAlevin.Fig3_UMAP.ManualDR.RealPCASpace.PCAtfidfprojected.20230320.Rdata", out_directory))
...
Alevin.UMAPtfidf.projected <- predict(scDesign2_umap_fit, t(Alevin.pcatfidf))
cellranger.UMAPtfidf.projected <- predict(scDesign2_umap_fit, t(cellranger.pcatfidf))
save(scDesign2_umap_fit, scDesign2.UMAPtfidf, umitools.UMAPtfidf.projected, Alevin.UMAPtfidf.projected, cellranger.UMAPtfidf.projected, file=sprintf("%s/BenchmarkUMI.withAlevin.Fig3_UMAP.ManualDR.RealPCASpace.UMAPtfidfprojected.20230320.Rdata", out_directory))

# Dataframe for projected UMAP
load(sprintf("%s/BenchmarkUMI.synthetic_cell_barcode.formatted.20230320.Rdata", out_directory))
umap_dat.projected.tfidf <- data.frame(
  rbind(scDesign2.UMAPtfidf, umitools.UMAPtfidf.projected, Alevin.UMAPtfidf.projected, cellranger.UMAPtfidf.projected))
colnames(umap_dat.projected.tfidf) <- c('x', 'y')
umap_dat.projected.tfidf$labels <- rep(clustering_synthetic_cluster[intersect_cell_ID], 4)
...
umap_dat.projected.tfidf$panels <- c(rep("ground truth", length(clustering_synthetic_cluster[intersect_cell_ID])),
                                     ...
                                     rep("Alevin", length(clustering_synthetic_cluster[intersect_cell_ID])),
...
umap_dat.projected.tfidf$panels <- factor(umap_dat.projected.tfidf$panels,
                                          levels = c("ground truth", "cellranger", "Alevin", "UMI-tools"))
save(umap_dat.projected.tfidf, file=sprintf("%s/BenchmarkUMI.withAlevin.Fig3_UMAP.ManualDR.RealPCASpace.UMAPtfidfprojected.20230320.Rdata", out_directory))
...
subset_label_list <- list(c("ground truth", "cellranger"), c("ground truth", "Alevin"), c("ground truth", "UMI-tools"))
...
save(mlisi_umap_mixed, file=sprintf("%s/BenchmarkUMI.withAlevin.Fig3_UMAP.ManualDR.RealPCASpace.UMAPtfidfprojected.miLISIvalues.20230320.Rdata", out_directory))

# Plot
load(sprintf("%s/BenchmarkUMI.withAlevin.Fig3_UMAP.ManualDR.RealPCASpace.UMAPtfidfprojected.20230320.Rdata", out_directory))
...
ggsave(file=sprintf("%s/BenchmarkUMI.withAlevin.Fig3_UMAP.ManualDR.RealPCASpace.UMAPtfidfprojected.noLISI.20230320.pdf",out_directory), umap_mixed_plot1, height=9, width = 9.6)
...
# umap_dat <- data.frame(
#   rbind(umap_result_scDesign2_data, umap_result_simu_umi, umap_result_simu_alevin, umap_result_simu_cellranger))
# colnames(umap_dat) <- c('x', 'y')
# umap_dat$labels <- rep(clustering_synthetic_cluster[intersect_cell_ID], 4)
...
#   rbind(umap_result_scDesign2_data, umap_result_simu_umi_rotate, umap_result_simu_alevin_rotate, umap_result_simu_cellranger_rotate))
# colnames(umap_dat_rotate) <- c('x', 'y')
# umap_dat_rotate$labels <- rep(clustering_synthetic_cluster[intersect_cell_ID], 4)
...
#                            "[Pearson Cor. = ",
#                            format(round(c(cor_cellranger.global, cor_Alevin.global, cor_umitools.global), 3), nsmall = 3), "]"))
...
# save(umap_dat, file=sprintf("%s/BenchmarkUMI_Fig3_UMAP.ManualDR.fixseed_20221014.Rdata", out_directory))
# save(umap_dat_rotate, file=sprintf("%s/BenchmarkUMI_Fig3_UMAP.ManualDR.fixseed.rotated_20221014.Rdata", out_directory))
# 
# 
# ### plotting
# load(file=sprintf("%s/BenchmarkUMI_Fig3_UMAP.ManualDR.fixseed.rotated_20221014.Rdata", out_directory))
# 
# point_size <- 0.25; point_transp <- 0.8
# get_colScale <- function(labels){
#   cbPalette <- c("#999999", "#E69F00", "#56B4E9", "#009E73",
...
#   xlab("UMAP 1") + ylab("UMAP 2")
# ggsave(file=sprintf("%s/BenchmarkUMI_Fig3_UMAP.ManualDR.fixseed.rotated.20221014.pdf",out_directory), umap_mixed_plot1, height = 9.6, width = 9.6, dpi = 600)
...
alevin.varyCell <- c(459, 486, 495, 498, 556)
alevin.varySeq <- c(142, 256, 488, 926, 1819)
...
time_dat <- data.frame(time=c(cellranger.varyCell, cellranger.varySeq,
                              alevin.varyCell, alevin.varySeq,
                              umitools.varyCell, umitools.varySeq),
                       case=rep(c(varyCell, varySeq), 3),
                       method=c(rep("cellranger", 10),
                                rep("Alevin", 10),
                                rep("UMI-tools", 10)),
                       measure=rep(c(rep("Cell number", 5), rep("Sequence depth", 5)), 3))
time_dat$method <- factor(time_dat$method, levels=c("cellranger", "Alevin", "UMI-tools"))
time_dat$measure <- factor(time_dat$measure, levels=c("Cell number", "Sequence depth"))
time_dat$case <- factor(time_dat$case, levels=c(varyCell, varySeq))
save(time_dat, file=sprintf("%s/BenchmarkUMI_Fig4_TIME.20221018.Rdata", out_directory))

load(file=sprintf("%s/BenchmarkUMI_Fig4_TIME.20221018.Rdata", out_directory))
group.colors <- c("#D55E00", "#E69F00", "#56B4E9", "#009E73") # groudtruth, cellranger, Alevin, UMI-tools
time_plot <- time_dat %>%
  ...
  scale_color_manual(values=group.colors[-1]) +
  theme_bw() +
  theme(
    # axis.line = element_line(colour = "black",size = 1,lineend = "square"),
...
# ggsave(file=sprintf("%s/BenchmarkUMI_Combined_20221014_withAlevin.pdf",out_directory), p_final, height = 9.6, width = 9.6, dpi = 600)
ggsave(file=sprintf("%s/BenchmarkUMI_Combined_20230224_withAlevin.pdf",out_directory), p_final, height = 9.6, width = 9.6, dpi = 600)
